...
Peninsula has three implementation layers:

1. Foundation model and agent runtime (e.g., Claude Code and other model connectors)
2. Peninsula platform layer (knowledge graph, orchestration, integrations, security controls)
3. Client intelligence layer (firm taxonomy, precedents, templates, and workflow patterns)

### 1.2 Base Platform vs Custom Intelligence

Base platform capabilities provide immediate value:
...
Custom intelligence modules deepen value by encoding firm-specific reasoning, style, and regulatory monitoring patterns.

### 1.3 Why This Matters for EBLG

EBLG's strategic need is not generic AI usage.  
It is preservation and activation of 35 years of institutional expertise in a form that can be inspected, reused, and defended over time.
